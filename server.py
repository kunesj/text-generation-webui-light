...
os.environ.update(
    {"GRADIO_ANALYTICS_ENABLED": "False", "BITSANDBYTES_NOWELCOME": "1", "GRADIO_TEMP_DIR": str(gradio_temp_path)}
)
...
def create_interface():
    ...
    shared.persistent_interface_state.update(
        {"mode": shared.settings["mode"], "loader": shared.args.loader or "llama.cpp", "filter_by_loader": "llama.cpp"}
    )

    if shared.settings["prompt-notebook"]:
        prompt = load_prompt(shared.settings["prompt-notebook"])
        shared.persistent_interface_state.update({"textbox-default": prompt, "textbox-notebook": prompt})
    ...
    with gr.Blocks(css=css, analytics_enabled=False, title=title, theme=ui.theme) as shared.gradio["interface"]:
        # Interface state
        shared.gradio["interface_state"] = gr.State(dict.fromkeys(shared.input_elements))

        # Audio notification
        if Path("user_data/notification.mp3").exists():
            shared.gradio["audio_notification"] = gr.Audio(
                interactive=False, value="user_data/notification.mp3", elem_id="audio_notification", visible=False
            )
...
if __name__ == "__main__":
    ...
    if shared.model_name != "None":
        ...
        if (
            "gpu_layers" not in shared.provided_arguments
            and shared.args.loader == "llama.cpp"
            and "gpu_layers" in model_settings
        ):
