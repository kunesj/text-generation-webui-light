                ...
                with gr.Column():
                    with gr.Row():
                        ...
                        shared.gradio["reset_preset"] = gr.Button(
                            "Restore preset", elem_classes="refresh-button", interactive=True
                        )
                        shared.gradio["neutralize_samplers"] = gr.Button(
                            "Neutralize samplers", elem_classes="refresh-button", interactive=True
                        )

                with gr.Column():
                    shared.gradio["filter_by_loader"] = gr.Dropdown(
                        label="Filter by loader", choices=["llama.cpp"], value="llama.cpp", elem_classes="slim-dropdown"
                    )
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio["temperature"] = gr.Slider(
                                0.01, 5, value=shared.settings["temperature"], step=0.01, label="temperature"
                            )
                            ...
                            shared.gradio["dynamic_temperature"] = gr.Checkbox(
                                value=shared.settings["dynamic_temperature"], label="dynamic_temperature"
                            )

                            gr.Markdown("## Curve cutoff")
                            shared.gradio["min_p"] = gr.Slider(
                                0.0, 1.0, value=shared.settings["min_p"], step=0.01, label="min_p"
                            )
                            shared.gradio["top_n_sigma"] = gr.Slider(
                                0.0, 5.0, value=shared.settings["top_n_sigma"], step=0.01, label="top_n_sigma"
                            )
                            shared.gradio["top_p"] = gr.Slider(
                                0.0, 1.0, value=shared.settings["top_p"], step=0.01, label="top_p"
                            )
                            shared.gradio["top_k"] = gr.Slider(
                                0, 200, value=shared.settings["top_k"], step=1, label="top_k"
                            )
                            shared.gradio["typical_p"] = gr.Slider(
                                0.0, 1.0, value=shared.settings["typical_p"], step=0.01, label="typical_p"
                            )
                            ...
                            shared.gradio["epsilon_cutoff"] = gr.Slider(
                                0, 9, value=shared.settings["epsilon_cutoff"], step=0.01, label="epsilon_cutoff"
                            )
                            shared.gradio["eta_cutoff"] = gr.Slider(
                                0, 20, value=shared.settings["eta_cutoff"], step=0.01, label="eta_cutoff"
                            )
                            shared.gradio["tfs"] = gr.Slider(
                                0.0, 1.0, value=shared.settings["tfs"], step=0.01, label="tfs"
                            )
                            shared.gradio["top_a"] = gr.Slider(
                                0.0, 1.0, value=shared.settings["top_a"], step=0.01, label="top_a"
                            )
                            ...
                            shared.gradio["frequency_penalty"] = gr.Slider(
                                0, 2, value=shared.settings["frequency_penalty"], step=0.05, label="frequency_penalty"
                            )
                            shared.gradio["presence_penalty"] = gr.Slider(
                                0, 2, value=shared.settings["presence_penalty"], step=0.05, label="presence_penalty"
                            )
                        ...
                        with gr.Column():
                            ...
                            shared.gradio["mirostat_tau"] = gr.Slider(
                                0, 10, step=0.01, value=shared.settings["mirostat_tau"], label="mirostat_tau"
                            )
                            shared.gradio["mirostat_eta"] = gr.Slider(
                                0, 1, step=0.01, value=shared.settings["mirostat_eta"], label="mirostat_eta"
                            )
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio["stream"] = gr.Checkbox(
                                value=shared.settings["stream"], label="Activate text streaming"
                            )
...
def create_event_handlers():
    ...
    shared.gradio["grammar_file"].change(
        load_grammar, gradio("grammar_file"), gradio("grammar_string"), show_progress=False
    )
