...
with open(Path(__file__).resolve().parent / "../css/NotoSans/stylesheet.css", encoding="utf-8") as f:
    css = f.read()
with open(Path(__file__).resolve().parent / "../css/main.css", encoding="utf-8") as f:
    css += f.read()
with open(Path(__file__).resolve().parent / "../css/katex/katex.min.css", encoding="utf-8") as f:
    css += f.read()
with open(Path(__file__).resolve().parent / "../css/highlightjs/highlightjs-copy.min.css", encoding="utf-8") as f:
    css += f.read()
with open(Path(__file__).resolve().parent / "../js/main.js", encoding="utf-8") as f:
    js = f.read()
with open(Path(__file__).resolve().parent / "../js/global_scope_js.js", encoding="utf-8") as f:
    global_scope_js = f.read()
with open(Path(__file__).resolve().parent / "../js/save_files.js", encoding="utf-8") as f:
    save_files_js = f.read()
with open(Path(__file__).resolve().parent / "../js/switch_tabs.js", encoding="utf-8") as f:
    switch_tabs_js = f.read()
with open(Path(__file__).resolve().parent / "../js/show_controls.js", encoding="utf-8") as f:
    show_controls_js = f.read()
with open(Path(__file__).resolve().parent / "../js/update_big_picture.js", encoding="utf-8") as f:
    update_big_picture_js = f.read()
with open(Path(__file__).resolve().parent / "../js/dark_theme.js", encoding="utf-8") as f:
...
def gather_interface_values(*args):
    ...
    output = {}
    for element, value in zip(interface_elements, args, strict=False):
    ...
    if (
        ...
        or (len(output["history"].get("visible", [])) == 0 and len(output["history"].get("internal", [])) == 0)
...
def save_settings(state, preset, extensions_list, show_controls, theme_state, manual_save=False):
    ...
    output["prompt-notebook"] = (
        state["prompt_menu-default"] if state["show_two_notebook_columns"] else state["prompt_menu-notebook"]
    )
    ...
    for element_name in change_elements:
        if element_name in shared.gradio:
            shared.gradio[element_name].change(
                gather_interface_values, gradio(shared.input_elements), gradio("interface_state")
            ).then(
