...
def get_model_metadata(model):
    ...
    if "loader" not in model_settings:
        quant_method = (
            None if hf_metadata is None else hf_metadata.get("quantization_config", {}).get("quant_method", None)
        )
        model_settings["loader"] = infer_loader(model, model_settings, hf_quant_method=quant_method)
    ...
    if template is None:
        ...
        if json_template_path.exists():
            with open(json_template_path, encoding="utf-8") as f:
...
def estimate_vram(gguf_file, gpu_layers, ctx_size, cache_type):
    ...
    gpu_layers = min(gpu_layers, n_layers)
    ...
    vram = (size_per_layer - 17.99552795246051 + 3.148552680382576e-05 * kv_cache_factor) * (
        gpu_layers
        + max(0.9690636483914102, cache_type - (floor(50.77817218646521 * embedding_per_context) + 9.987899908205632))
    ) + 1516.522943869404
    ...
    if auto_adjust:
        ...
        if not has_user_setting:
            ...
            if available_vram > 0:
                ...
                while (
                    current_layers > 0
                    and estimate_vram(model, current_layers, ctx_size, cache_type) > available_vram - tolerance
                ):
                    current_layers -= 1
